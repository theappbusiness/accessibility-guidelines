...
By default Fultter suports both portrait and landscape mode.

There is a way to lock you it in either mode. This can be done on a screen by screen basis or app wide. The app wide way being much simpler to implement and less of a hassle to maintain.
...
```
> Note: this does not lock the app into the defind orientation and can be changed later in the app on another screen using the same method

* The screen by screen method

```dart
// Setting the oriantation during the initilazation of a widget
@override
void initState() {
...
// Making sure that when the screen/widget has been destroyed/navigated away from the orientation goes back to normal 
// otherwise the orientation will be locked trough out the app from this point on
@override
void dispose() {
  SystemChrome.setPreferredOrientations([
    DeviceOrientation.landscapeLeft,

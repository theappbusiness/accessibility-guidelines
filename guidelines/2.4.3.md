...
* By default Flutter will focus things in the order that they are drawn on the screen from top down.
* There are ways of affecting this order for custom screen reader behaviour by passing a `SemanticSortKey` to the `sortKey` property of `Semantics()` where the lower value will be read first.

...
* For widgets that dont have this set by defualt and dont expose this semantic property you can do this:

```dart
Semantics(
  header: true,
  child: HeadingWidget() // The widget you wish to set as a heading
  )
```
...
>Note: if you set `textField: true` in the `Semantics()` widget with a `TextField()` child then the screen reader will only pick up the semantics of the `TextField()` and the `Semantics()` widget will be ignored. You can always `MergeSemantics()` to remedy this.

### Setting a custom widget as a tab bar:

* Only `BottomNavigationBarItem()` widgets are allowed in a `BottomNavigationBar()` widget but if you want/need more control then what is provided by default you can use one of the examples bellow
...
BottomNavigationBarItem(
          ...
          // this would make the CustomWidget() more reusable and make for cleaner code 
            icon: Semantics(label: 'CustomWidget Semantic Lable',child: CustomWidget()),
),
...
BottomNavigationBarItem(
            icon: Icon(Icons.icon), // icon is a requierd property
            title: Text(
              'Text',
              semanticsLabel: 'Text Semantic Label',
            ),
),
// If you wish to exlude any default Semantic such as Text() you could do this 
BottomNavigationBarItem(
...
```
> Note: You cannot exclude/disable the semantcis of `BottomNavigationBarItem()` since it cant be wrapped in a `ExcludeSemantics()` or `Semantics(excludeSemantics:true)` this is a flaw in the design of the `BottomNavigationBar()` and would need a custom bottom navigation solution in order to remedy.

### Dealing with invisible widgets and their semantics:

* If you have a widget that you want to hide from the user by making it invisible whether permenantly or based on the state of the app. Or if it is a widget that holds no semantic value to a accessibility user such as a asscent or decoration and would only hinder a accessibility user with useless information or disturb their navigation journey you can do as bellow.
...
*There are two ways to add a lable to the text field both are shown bellow one of them is the same as setting a custom view as a search field:
...
TextField(
  decoration: InputDecoration(
    hintText: 'Testing hint', // hint text has its built in semantics
    labelText: labelText.toUpperCase(), // lable text has its own builtsemantics
  ),
),
```

### More guidance for Flutter 
* [Flutter accessibility developer reference](https://flutter.dev/docs/development/accessibility-and-localization/accessibility "flutter.dev documentation on buildng accessibility")

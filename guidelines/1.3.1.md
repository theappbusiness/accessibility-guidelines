...
## Your page conveys information, structure and relationships visually.<br/>&rarr; Make sure those are described in code in ways that assistive technologies understand.
...
- Using the `placeholder` attribute to provide an ['Accessible Name'](./definitions.md#accessibility-name) for a text `input` field. (The `placeholder` attribute is not well supported by some browsers and assistive technologies combinations, and shouldn't be used for labelling text fields).
...
| `<form>`    | A form                                                                                                    | `role="form"` if it has an [Accessible Name](./definitions.md#accessibility-name)   |
| `<section>` | A section of content that you want added in the list of Landmark regions                                  | `role="region"` if it has an [Accessible Name](./definitions.md#accessibility-name) |
...
- If a same HMTL sectioning element and/or equivalent ARIA landmark `role` is used multiple times on a page, make sure that each instance has a unique [Accessible Name](./definitions.md#accessibility-name).

...
The `BottomNavigationBar(items:[])` only takes a list of `BottomNavigationBarItem` so the item widegts cannot be wrapped in a `Semantics()` widget but their children can.
...
If you have widgets that you want to exclude from semantics because they serve no purpose for a visually impared user or if they are hidden until a change of state you can use the `ExcludeSemantics()` or `Semantics(excludeSemantics: bool)` as shown bellow:
...
TextField(
  decoration: InputDecoration(
    hintText: String, // hint text has its built in semantics
    labelText: String, // lable text has its own builtsemantics
  ),
),
```

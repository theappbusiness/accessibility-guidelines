...
If a widget goes of screen on a device it will show a overflow error on the screen during development.

Text in `Text()` widgets will naturally be softwtaped but can have this functionality toggled.
```dart 
...
Scrolling diraction is locked to a default direction but can have this toggled.
